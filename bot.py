...
token = "ODU1MDQzNTUwNzg5NjMyMDIx.YMsvXA.fywyg48GVbnd-g-TTrZwGXFkcTM" # Insert your bot token here
intents = discord.Intents.default()
intents.members = True
bot = commands.Bot(command_prefix="gg-", activity=discord.Game(name=f'''g-info | g-commands'''), intents=intents)
...
async def on_command_error(ctx, error): # Error handler
    if isinstance(error, commands.CommandNotFound): # If command is not found
	    ...
	    .set_footer(text="Created using bit.ly/gwendalitosassistant")
    ...
    if isinstance(error, commands.MissingRequiredArgument): # If command is missing an argument
        coolerrr1 = (ctx.author.mention) + "**Missing required arguments!**"
        await ctx.send(coolerrr1)
        return
...
async def suggest(ctx, *, suggestion: str): # Suggestion command, the category and the suggestion get split by a comma
	...
	channel = bot.get_channel(887623408798556190)
...
async def bug(ctx, *, bug_report: str): # Bug report command, the category and the bug gets split by a comma
	await ctx.message.delete()
	channel = bot.get_channel(887623408798556190) # Bot gets the bug reports channel of gwendalito's support server
	upvote = "<:upvote:879397319475343420>"
...
@bot.command()
@commands.cooldown(1, 60, commands.BucketType.user)
async def contact(ctx, *, message = None): # Command to send a DM to gwendal (me lmao)
	await ctx.message.delete()
